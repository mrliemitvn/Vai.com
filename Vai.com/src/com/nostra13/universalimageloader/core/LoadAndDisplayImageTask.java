...
import com.nostra13.universalimageloader.core.assist.ProgressImageLoadingListener;
...
final class LoadAndDisplayImageTask implements Runnable {
    ...
    private static final String LOG_TASK_CANCELLED_IMAGEVIEW_REUSED =
        "ImageView is reused for another image. Task is cancelled. [%s]";
    private static final String LOG_TASK_CANCELLED_IMAGEVIEW_LOST =
        "ImageView was collected by GC. Task is cancelled. [%s]";
    private static final String LOG_TASK_INTERRUPTED = "Task was interrupted [%s]";

    private static final String ERROR_PRE_PROCESSOR_NULL = "Pre-processor returned null [%s]";
    private static final String ERROR_POST_PROCESSOR_NULL = "Pre-processor returned null [%s]";
    private static final String ERROR_PROCESSOR_FOR_DISC_CACHE_NULL =
        "Bitmap processor for disc cache returned null [%s]";
    private static final String ERROR_DOWNLOAD_RETRY = "Error when download, retry [%s]";
    ...
    public LoadAndDisplayImageTask(ImageLoaderEngine engine, ImageLoadingInfo imageLoadingInfo, Handler handler) {
        ...
        if (listener instanceof ProgressImageLoadingListener) {
            hasProgress = true;
        } else {
            hasProgress = false;
        }
    ...
    /**
     * Check whether target ImageView wasn't collected by GC and the image URI of this task matches to image URI which
     * is actual
     * for current ImageView at this moment and fire
     * {@link ImageLoadingListener#onLoadingCancelled(String, android.view.View)} event if it doesn't.
     */
    ...
    private Bitmap decodeImage(String imageUri) throws IOException {
        ...
        ImageDecodingInfo decodingInfo =
            new ImageDecodingInfo(memoryCacheKey, imageUri, targetSize, viewScaleType, getDownloader(), options);
    ...
    private String tryCacheImageOnDisc(File targetFile) {
        ...
        while (true) {
            try {
                ...
                retry++;
                if (retry < MAX_RETRY) {
                    L.e(ERROR_DOWNLOAD_RETRY, retry + "");
                } else {
                    return uri;
                }
            }
    ...
    private boolean downloadSizedImage(File targetFile, int maxWidth, int maxHeight) throws IOException {
        ...
        DisplayImageOptions specialOptions =
            new DisplayImageOptions.Builder().cloneFrom(options).imageScaleType(ImageScaleType.IN_SAMPLE_INT).build();
        ImageDecodingInfo decodingInfo =
            new ImageDecodingInfo(memoryCacheKey, uri, targetImageSize, ViewScaleType.FIT_INSIDE, getDownloader(),
                specialOptions);
    ...
    private void downloadImage(File targetFile) throws IOException {
        ...
        InputStream is = getDownloader().getStream(uri, ref);
        try {
            OutputStream os = new BufferedOutputStream(new FileOutputStream(targetFile), BUFFER_SIZE);
            try {
                int size = 1;
    ...
    private void copyStream(InputStream is, OutputStream os, int size) throws IOException {
        ...
        while (true) {
            ...
            if (hasProgress) {
                final int percent = (int) ((num / (float) size) * 100);
                if (!checkTaskIsNotActual()) {
                    handler.post(new Runnable() {
                        @Override
                        public void run() {
                            ImageView imageView = checkImageViewRef();
                            if (imageView != null) {
                                ((ProgressImageLoadingListener) listener).onDownloadProgress(uri, imageView, percent);
                            }
                        }
                    });
                }
